...
class UserMessage extends React.Component {
    ...
    let config2 = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${BASE_URL}user/support/getByUser/${localStorage.getItem('rider_support')}`,
      headers: {},
    };
  ...
  handleSubmit = (event) => {
    ...
    if (socket) {
        ...
        const message= {
          'text':text,
          'sender':localStorage.getItem('user_support'),
          'sendTo':'user',
          'type':'admin',
        }
            ...
            {this.state.users.map((item,index)=>{
                return (
                    ...
                    <li className="active" onClick={()=>{
                      localStorage.setItem('user_support',item?.id)
                      this.setState({getSupport:item?.id});

                      let config3 = {
                        method: "get",
                        maxBodyLength: Infinity,
                        url: `${BASE_URL}user/getById/${item?.id}`,
                        headers: {},
                      };
